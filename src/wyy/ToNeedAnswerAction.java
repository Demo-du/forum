...
public class ToNeedAnswerAction extends TipAndFenYeAction{
    private List content;		//显示内容
    @Override
    public String getSql(){
    	String uid = (String)getSession().get("uid");		//得到登录人的ID 
    	System.out.println(getSession().get("course"));   	
		sql = "select a.tid,a.gname,a.ftr,a.title,a.djs,"+
			...
			" dta.TGID=dtg.TGID and dta.TID not in "+
			"(select distinct TID from DY_Revert where UID="+
			uid+") and dtg.TGID "+getCourse()+
			...
			"dra.UID and b.tid not in (select "+
