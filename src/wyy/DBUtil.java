...
public class DBUtil{
	...
	public List getIndexContent(String sql){
		...
		try{
			sql = new String(sql.getBytes(),"iso8859-1");		//对SQL进行转码
			...
			if(rl.size()!=0){									//结果不为空的情况
				for(int i=0;i<rl.size();i++){					//遍历结果
					...
					gi.setTid(new String((byte[])map.get("tid"),"iso8859-1"));	//设置最后回复问题的ID值
		...
		try{
			...
			if(rl.size()!=0){						//结果不为空的情况
				for(int i=0;i<rl.size();i++){		//遍历结果
					...
					ai.setTgname(new String(map.get("TGName").toString().getBytes("iso8859-1"),"gbk"));
					ai.setReason(new String(map.get("AReason").toString().getBytes("iso8859-1"),"gbk"));
					ai.setFlag(map.get("AFlag").toString());
					ai.setStatus(new String(map.get("AStatus").toString().getBytes("iso8859-1"),"gbk"));
					al.add(ai);
				}
			}
		}
		...
		try{
			...
			if(rl.size()!=0){						//结果不为空的情况
				for(int i=0;i<rl.size();i++){		//遍历结果
					...
					tdi.setZhdl(new String((byte[])map.get("zhdl")));//设置tdi的发帖人最后登陆时间属性
		...
		try{
			...
			if(rl.size()!=0){						//结果不为空的情况
				for(int i=0;i<rl.size();i++){		//遍历结果
					...
					ci.setUname(new String(map.get("UName").toString().getBytes("iso8859-1"),"gbk"));				
					al.add(ci);
				}
			}
		}
	...
	public boolean update(String sql){							//执行单个SQL语句的方法
		boolean flag = false;
		try{
			sql = new String(sql.getBytes(),"iso8859-1");//对SQL语句进行转码
